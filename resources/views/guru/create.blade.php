...
@section('content')
<h1 class="text-2xl font-semibold mb-4 text-gray-100">Tambah Guru</h1>
<form action="{{ route('guru.store') }}" method="POST" class="bg-gray-800/60 p-6 rounded shadow space-y-4 border border-gray-700">
    @csrf
    <div>
        <label class="block text-sm mb-1 text-gray-300">Nama Guru</label>
        <input type="text" name="nama_guru" value="{{ old('nama_guru') }}" class="w-full bg-gray-900 border border-gray-700 rounded px-3 py-2 text-gray-100" required />
    </div>
    <div>
    ...
    <div class="grid md:grid-cols-2 gap-4">
        ...
        <input type="text" name="mapel" value="{{ old('mapel') }}" class="w-full bg-gray-900 border border-gray-700 rounded px-3 py-2 text-gray-100" required />
        </div>
    </div>
