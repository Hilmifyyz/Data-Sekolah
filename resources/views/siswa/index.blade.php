...
<div class="flex items-center justify-between mb-4">
    ...
    <a href="{{ route('siswa.create') }}" class="px-4 py-2 rounded bg-blue-600 text-white hover:bg-blue-700">Tambah Siswa</a>
</div>
            ...
            @forelse ($siswas as $i => $siswa)
                <tr class="border-t border-gray-700">
                    ...
                    <td class="px-4 py-2 flex gap-2">
                        <a href="{{ route('siswa.edit', $siswa) }}" class="px-3 py-1.5 rounded bg-yellow-500 text-white hover:bg-yellow-600">Edit</a>
                        <form action="{{ route('siswa.destroy', $siswa) }}" method="POST" onsubmit="return confirm('Hapus data ini?')">
                            @csrf
                            @method('DELETE')
